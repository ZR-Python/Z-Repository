...
with open("ZombiePhrases.txt", "r") as f:
    zombie_phrases = f.readlines()
zombie_phrases = [line.strip() for line in zombie_phrases]
f.close()
	...
	def move_forward(self):
		...
		if (abs(Phoebe.rect.center[0] - self.rect.center[0]) > 30):
			...
			if (level_wave == 2):
				self.rect.center = (self.rect.center[0] + (5*self.run_speed), self.rect.center[1])
			if (level_wave == 3):
				self.rect.center = (self.rect.center[0] - (3*self.run_speed*self.wave_three_status), self.rect.center[1])
...
Bridge_Level = Background("Castle.png")
...
while (run):
	for event in pygame.event.get():
		...
		if event.type == pygame.KEYDOWN:
			...
			if event.key == pygame.K_BACKSPACE:
				...
				if event.unicode.islower() or event.unicode.isupper() or event.unicode.isdigit() or event.unicode == ',' or event.unicode == "?" or event.unicode == " " or event.unicode == "'":
	...
	if (level_spawn_timer % 60 == 0 and level_spawn_timer > -1):
		...
		if (zombie_type == 1):
			...
			New_Zombie = Zombie("Fat Zombie.png", zombie_phrases[(random.randint(0, len(zombie_phrases)-1))])
			New_Zombie.load_fat()
			New_Zombie.run_speed = zombie_speed*.25
		Enemy_List.insert(0, New_Zombie)
		Enemies.add(New_Zombie)
		if level_wave == 1:
			New_Zombie.rect.center = (1060, 480)
		elif level_wave == 2:
			New_Zombie.rect.center = (160, 480)
			...
			if wave_three_rotating_sides %2 == 0:
				...
				New_Zombie.rect.center = (130, 480)
				New_Zombie.flip_sides()
				wave_three_rotating_sides += 1
				New_Zombie.wave_three_status = -1
