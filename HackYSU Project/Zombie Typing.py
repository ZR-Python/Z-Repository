		...
		if self.eating == True:
			if self.eat_timer == 100:
				...
				if (Phoebe.get_health() == 0):
					...
					mixer.music.load("Defeat.wav")
					mixer.music.play(-1)
					#winsound.PlaySound("Defeat.wav", winsound.SND_ASYNC | winsound.SND_LOOP)
				self.eat_timer = 0
			else:
				self.eat_timer += 1
...
BossBoxes = []
mixer.music.load("TitleScreen.wav")
mixer.music.play(-1)
#winsound.PlaySound("TitleScreen.wav", winsound.SND_ASYNC | winsound.SND_LOOP)
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.MOUSEBUTTONDOWN:
			if (Play_Button.rect.collidepoint(pygame.mouse.get_pos())):
				...
				gunshot_sound.play()
				title_screen = False
				mixer.music.load("Forest.wav")
				mixer.music.play(-1)
				#winsound.PlaySound("Forest.wav", winsound.SND_ASYNC | winsound.SND_LOOP)
				#TEMP CODE
				for button in Number_of_Zombies_Buttons:
...
while (run):
	...
	if (current_stage == 3):
		if Phoebe.kill_count == 13:
			...
			victory = True
			total_kill_count += Phoebe.kill_count
			mixer.music.load("Victory.wav")
			mixer.music.play(-1)
			#winsound.PlaySound("Victory.wav", winsound.SND_ASYNC | winsound.SND_LOOP)
		for box in BossBoxes:
			box.display_text()
	...
	if (Phoebe.kill_count == number_of_zombies * 4 and current_stage != 3):
		...
		current_stage += 1
		if (current_stage == 1):
			mixer.music.load("Bridge.wav")
			mixer.music.play(-1)
			#winsound.PlaySound("Bridge.wav", winsound.SND_ASYNC | winsound.SND_LOOP)
		if (current_stage == 2):
			mixer.music.load("Castle.wav")
			mixer.music.play(-1)
			#winsound.PlaySound("Castle.wav", winsound.SND_ASYNC | winsound.SND_LOOP)
		if (current_stage == 3):
			mixer.music.load("BossFight.wav")
			mixer.music.play(-1)
			#winsound.PlaySound("BossFight.wav", winsound.SND_ASYNC | winsound.SND_LOOP)
			Characters.empty()
